...
export const createPackageHandlers = ({ installPackages }) => ({
  ...
  toast: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-hot-toast'], { cwd: projectPath }, 'Failed to install react-hot-toast')
    }
  ...
  printer: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-to-print', 'socket.io-client'], { cwd: projectPath }, 'Failed to install react-to-print')
    }
    await configureSocket(projectPath)
