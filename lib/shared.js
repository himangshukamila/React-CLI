...
export const apiContent = `import axios from 'axios'
...
export const getHumanReadableError = (error) => {
  if (typeof navigator !== 'undefined' && !navigator.onLine || error.code === 'ERR_NETWORK') {
  ...
  if (status === 400 || status === 422) {
    return data?.message || "Please check the input fields and try again."
  }
  if (status === 401) {
    if (typeof localStorage !== 'undefined') {
  ...
  if (status === 403) {
    return "You don't have access to do this. Contact your administrator."
  }
  if (status === 404) {
    return data?.message || "The requested item was not found or may have been deleted."
  }
  if (status === 405) {
    return "This action is not supported for this resource."
  }
  if (status === 407) {
    return "Proxy authentication required. Please authenticate with your proxy server."
  }
  if (status === 408) {
    return "Request timed out. The server took too long to respond."
  }
  if (status === 409) {
    return data?.message || "User or record already exists with this information."
  }
  if (status === 413) {
    return "The uploaded file or request data is too large."
  }
  if (status === 415) {
    return "File format or media type is not supported."
  }
  if (status === 429) {
    return "Too many attempts. Try again in 30 seconds."
  }
  if (status === 502 || status === 503 || status === 504) {
    return "The server is temporarily unavailable or under maintenance. Please try again shortly."
  }
  if (status && status >= 500) {
    return "Something went wrong on server end. Please try again in a moment."
  }

  return error.message || "An unexpected error occurred."
}

// Global Response Interceptor for Auto Toast Notifications
...
export const api = {
  // GET request: api.get('/api/name', config)
  ...
  // POST request: api.post('/api/name', data, config)
...
export const configureAxios = async (projectPath = process.cwd()) => {
  ...
  await ensureDir(path.join(projectPath, 'src', 'services'))
  await writeFile(path.join(projectPath, 'src', 'services', 'api.js'), apiContent)
}

export const configureApiMethods = async (methods = [], projectPath = process.cwd()) => {
  const apiPath = path.join(projectPath, 'src', 'services', 'api.js')
  ...
  const methodTemplates = {
    put: `  // PUT request: api.put('/api/name', data, config)
  put: async (url, data = {}, config = {}) => {
    const response = await axiosInstance.put(url, data, config)
    console.log('API PUT response:', response.data)
    return response.data
  },`,
    patch: `  // PATCH request: api.patch('/api/name', data, config)
  patch: async (url, data = {}, config = {}) => {
    const response = await axiosInstance.patch(url, data, config)
    console.log('API PATCH response:', response.data)
    return response.data
  },`,
    delete: `  // DELETE request: api.delete('/api/name', config)
  ...
  for (const method of methods) {
    ...
    if (methodTemplates[lowerMethod]) {
      const regex = new RegExp(`\\b${lowerMethod}\\s*:\\s*async\\b`)
  ...
  if (addedMethods.length > 0) {
    ...
    console.log(chalk.yellow('\nℹ Requested method(s) are already present in src/services/api.js'))
  }
}
