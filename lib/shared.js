...
const toastContainerImport = "import { ToastContainer } from 'react-toastify'"
const toastCssImport = "import 'react-toastify/dist/ReactToastify.css'"
const toastContainerImportRegex = /import\s*{\s*ToastContainer\s*}\s*from\s*['"]react-toastify['"]/
const toastCssImportRegex = /import\s*['"]react-toastify\/dist\/ReactToastify\.css['"]/
...
export const pathExists = async (targetPath) => {
  try {
    ...
    throw new Error(`Could not check path: ${targetPath}. ${error.message}`)
  }
}
...
export const applyToastToApp = async (projectPath) => {
  ...
  if (!toastCssImportRegex.test(content)) {
    content = `${toastCssImport}\n${content}`
  }
  ...
  if (content.includes(sentinel)) {
    await writeFile(appPath, content.replace(sentinel, '      <ToastContainer />'))
    return
  }
...
${returnedJsx}
  ...
  throw new Error('Could not safely update App.jsx for React Toastify')
}
...
export const configureTailwind = async (projectPath) => {
  ...
  await writeFile(path.join(projectPath, 'vite.config.js'), viteConfigContent)
  await writeFile(path.join(projectPath, 'src', 'index.css'), tailwindCssContent)
  await ensureImportLine(path.join(projectPath, 'src', 'main.jsx'), "import './index.css'")
}
...
export const createPackageHandlers = ({ installPackages }) => ({
  ...
  axios: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'axios'], { cwd: projectPath }, 'Failed to install axios')
    }
  },
  socket: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'socket.io-client'], { cwd: projectPath }, 'Failed to install socket.io-client')
    }
    await configureSocket(projectPath)
  },
  toast: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-toastify'], { cwd: projectPath }, 'Failed to install react-toastify')
    }
    await configureToast(projectPath)
  },
  icon: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-icons'], { cwd: projectPath }, 'Failed to install react-icons')
    }
  },
  lucide: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'lucide-react'], { cwd: projectPath }, 'Failed to install lucide-react')
    }
  },
  router: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-router-dom'], { cwd: projectPath }, 'Failed to install react-router-dom')
    }
  },
  qr: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-qr-code'], { cwd: projectPath }, 'Failed to install react-qr-code')
    }
  },
  webcam: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-webcam'], { cwd: projectPath }, 'Failed to install react-webcam')
    }
    await configureWebcam(projectPath)
  },
  printer: async (projectPath) => {
    if (installPackages) {
      await runCommand('npm', ['install', 'react-to-print'], { cwd: projectPath }, 'Failed to install react-to-print')
    }
  },
})
