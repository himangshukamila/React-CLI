...
export const generateApiFileContent = (methods = ['get', 'post']) => {
...
const getFieldErrors = (error) => {
  ...
  if (Array.isArray(data.details)) {
    return data.details.reduce((acc, d) => ({ ...acc, [d.field ?? d.path]: d.message }), {})
  }
  return null
}
...
export const configureApiMethods = async (methods = ['get', 'post'], projectPath = process.cwd()) => {
  const pkgJsonPath = path.join(projectPath, 'package.json')
  if (await pathExists(pkgJsonPath)) {
    const pkgJson = JSON.parse(await readFile(pkgJsonPath, 'utf8'))
    const allDeps = { ...(pkgJson.dependencies || {}), ...(pkgJson.devDependencies || {}) }
  ...
  if (!(await pathExists(apiPath))) {
    await ensureDir(path.join(projectPath, 'src', 'services'))
    const initialContent = generateApiFileContent(requestedMethods)
    await writeFile(apiPath, initialContent)
    console.log(chalk.green(`\n✔ Created src/services/api.js with method(s): ${requestedMethods.map((m) => m.toUpperCase()).join(', ')}!`))
    return
  }
