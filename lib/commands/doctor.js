...
export const doctor = async () => {
  try {
    ...
    await typeText(`${muted('node')}     ${strong(process.version)}`)

    try {
      const npmVersion = await runCommand('npm', ['--version'], { stdio: 'pipe' }, 'Could not read npm version')
      await typeText(`${muted('npm')}      ${strong(npmVersion.stdout.trim())}`)
    } catch (error) {
    ...
    if (await pathExists(envPath)) {
      ...
      warn('.env missing', 'run react env add VITE_SERVER_URL http://localhost:3000')
    }
