...
export const configureFormBoilerplate = async (rawArgs = []) => {
  try {
    ...
    const lucideIconsUsed = ['Send', 'CheckCircle2', 'XCircle']
    if (hasPasswordField) {
    ...
    const fieldBlocks = fields.map((f) => {
      ...
      const labelSizeClass = typeof labelSize === 'string' && labelSize.startsWith('text-') ? labelSize : 'text-xs'
      const inputSizeClass = typeof inputSize === 'string' && inputSize.startsWith('text-') ? inputSize : 'text-sm'
        ...
        <div className="space-y-1.5 group">
          ...
          <div className="relative flex items-center">
            <input
              ...
              ${maxLenProp}
              style={inputStyle}
              className={\`w-full bg-zinc-900/90 text-zinc-100 ${inputSizeClass} rounded-xl pl-4 ${isPassword ? 'pr-10' : 'pr-4'} py-2.5 border border-zinc-800 focus:border-cyan-500/80 focus:ring-4 focus:ring-cyan-500/15 outline-none transition-all duration-200 placeholder:text-zinc-600\`}
            />
${isPassword ? `            <button
              type="button"
