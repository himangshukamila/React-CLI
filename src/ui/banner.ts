...
export const defaultFlagFolders: string[] = [
  "components",
  "pages",
  "hooks",
  "utils",
];
export const packageFlags: string[] = [
  "tailwind",
  "axios",
  "zod",
  "socket",
...
export const commandReference: [string, string][] = [
  ...
  [
    "zecron <name> --ui",
    "Configure app setup in a local browser setup wizard GUI",
  ],
  ["zecron .", "Create Vite + React app in the current directory"],
  [
    "zecron <name> --tailwind --axios",
    "Create an app non-interactively using package flags",
  ],
  ...
  [
    "zecron build",
    "Run production build (npm run build) with completion status",
  ],
  [
    "zecron build open",
    "Run production build and launch preview server (npm run preview)",
  ],
  ...
  [
    "zecron push --github",
    "Stage all changes, auto-generate commit message, and push",
  ],
  [
    "zecron set --font",
  ...
  [
    "zecron set ws",
    "Generate production-grade WebSocket client service in src/services/webSocket.js",
  ],
  [
  ...
  [
    "zecron make loader",
    "Generate responsive backdrop-blur Loader (src/components/Loader.jsx)",
  ],
  [
    "zecron make printer",
    "Generate Printer page (src/pages/Printer.jsx) with socket print queue",
  ],
  [
    "zecron set button",
...
export const packageOptions: OptionChoice[] = [
  ...
  },
  {
    value: "router",
    label: "React Router",
    hint: "react-router-dom + src/router",
  },
...
export const folderOptions: OptionChoice[] = [
  { value: "env", label: ".env", hint: "VITE_SERVER_URL boilerplate" },
  {
    value: "assets",
    label: "public/assets",
    hint: "create public/images and public/fonts",
  },
  {
    value: "components",
...
export const row = (label: string, value: string, hint: string = ""): void => {
  console.log(
    `${chalk.hex("#38BDF8")(label.padEnd(12))}${strong(value)} ${hint ? chalk.hex("#94A3B8")(` ${hint}`) : ""}`,
  );
};
...
export const printCommandReference = (): void => {
  ...
  const commandWidth =
    Math.max(...commandReference.map(([command]) => command.length)) + 2;
  commandReference.forEach(([command, purpose]) => {
    console.log(
      `${chalk.hex("#00E5FF").bold(command.padEnd(commandWidth))}${chalk.whiteBright(purpose)}`,
    );
  });
...
export const pass = (message: string, hint: string = ""): void => {
  console.log(
    `${chalk.hex("#10B981").bold("✅")} ${chalk.bold.whiteBright(message)} ${hint ? chalk.hex("#94A3B8")(hint) : ""}`,
  );
};

export const warn = (message: string, hint: string = ""): void => {
  console.log(
    `${chalk.hex("#F59E0B").bold("⚠️")} ${chalk.bold.yellowBright(message)} ${hint ? chalk.hex("#94A3B8")(hint) : ""}`,
  );
};

export const fail = (message: string, hint: string = ""): void => {
  console.error(
    `${chalk.hex("#EF4444").bold("❌")} ${chalk.bold.redBright(message)} ${hint ? chalk.hex("#94A3B8")(hint) : ""}`,
  );
  process.exit(1);
};

export const printCliHeader = ({
  displayName,
}: {
  displayName: string;
  ...
  console.log(
    `${strong("zecron")} ${muted("1.0.0")}  ${muted("·")}  ${muted(`node ${process.versions.node}`)}  ${muted("·")}  ${muted(process.platform)}`,
  );
  console.log(
    `${muted("type")} ${accent("help")} ${muted("to see commands, run")} ${accent(`zecron ${displayName}`)} ${muted("to begin.")}`,
  );
};
