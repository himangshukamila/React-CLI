...
import { section, pass, warn, fail, accent, typeText } from '../ui/banner.js'
...
export const writeEnvEntries = async (entries: Map<string, string>): Promise<void> => {
...
export const envList = async (): Promise<void> => {
  try {
    ...
    for (const [key, value] of entries) {
      await typeText(`${accent(key.padEnd(22))}${chalk.white(value)}`)
    }
  } catch (error: any) {
    fail(error.message)
  }
...
export const envAdd = async (key: string, value: string): Promise<void> => {
  try {
    validateEnvKey(key)
...
export const envRemove = async (key: string): Promise<void> => {
  try {
    validateEnvKey(key)
