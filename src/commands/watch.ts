...
import { section, fail, muted, strong } from '../ui/banner.js'
...
export const findWatchPort = (preferredPort: number): Promise<number> => findLocalPort(preferredPort)

export const sanitizeForTerminal = (value: any): string =>
...
export const printFrontendLog = (payload: FrontendLogPayload): void => {
  ...
  if (payload.response) {
    ...
    const responseStr = typeof payload.response === 'object' ? JSON.stringify(payload.response, null, 2) : String(payload.response)
    sanitizeForTerminal(responseStr)
      .split('\n')
...
export const watchFrontendLogs = async (): Promise<void> => {
  try {
