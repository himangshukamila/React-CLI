...
export const normalizeUiSelections = (payload: any): UiSelections => {
  ...
  if (payload.projectName && typeof payload.projectName === 'string' && payload.projectName.trim()) {
    const nameStr = payload.projectName.trim()
    if (!projectNameRegex.test(nameStr) || nameStr.includes('..') || nameStr.includes('/')) {
      throw new Error(`Invalid project name: ${nameStr}`)
    }
    projectName = nameStr
  }
...
export const readSetupUiStyle = async (): Promise<string> => {
  const cssPath = path.join(rootDir, 'templates', 'setup-ui', 'style.css')
  return readFile(cssPath)
}

export const startSetupWizardServer = async ({ displayName = '' }: SetupWizardOptions = {}): Promise<UiSelections> => {
  ...
  const selections = await new Promise<UiSelections>((resolve, reject) => {
    let resolved = false
