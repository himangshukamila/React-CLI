...
export const validatePathSegments = (segments: string[]): void => {
...
export const parseFolderTarget = (folderName: string): string[] => {
...
export interface ParsedMakeTarget {
  baseFolders: string[]
  folders: string[]
  baseName: string
}

export const parseMakeTarget = (folderName: string, name: string, subfolder?: string): ParsedMakeTarget => {
...
export const makeTemplates: Record<string, (name: string) => string> = {
...
export default ${name}
`,
  hook: (name: string) => `import { useState } from 'react'
...
`,
  service: (name: string) => `const ${name} = {
...
export default ${name}
`,
  default: (name: string) => `export const ${name} = () => {
  return null
}
`,
}

export interface MakeConfigItem {
  ext: string
  format: (name: string) => string
  template: string
}

export const makeConfig: Record<string, MakeConfigItem> = {
...
export const makeFolderKinds: Record<string, string> = {
  component: 'component',
...
export const ensureExistingFolder = async (folderPath: string): Promise<void> => {
...
export const makeFolder = async (folderName: string): Promise<void> => {
...
export const makeFile = async (folderName: string, name: string, subfolder?: string): Promise<void> => {
  try {
    ...
    const lastBaseFolder = target.baseFolders[target.baseFolders.length - 1] || ''
    const kind = makeFolderKinds[lastBaseFolder.toLowerCase()] || 'default'
