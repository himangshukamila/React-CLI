...
export const resolvePackageName = (packageName: string): string => {
...
let cachedPM: string | null = null
export const detectPackageManager = async (): Promise<string> => {
...
export const ensureViteScriptsInPackageJson = async (projectPath: string): Promise<void> => {
...
export const createViteApp = async (commandTarget?: string, projectPath?: string): Promise<void> => {
...
export const readDir = async (targetPath: string): Promise<string[]> => {
  try {
...
export const readFile = async (targetPath: string): Promise<string> => {
  try {
...
export const writeFile = async (targetPath: string, content: string): Promise<void> => {
  try {
...
export const stat = async (targetPath: string): Promise<fs.Stats> => {
  try {
    return await fsPromises.stat(targetPath)
  } catch (error: any) {
    throw new Error(`Could not stat path: ${targetPath}. ${error.message}`)
  }
}

export const readTemplate = async (...segments: string[]): Promise<string> => {
  return readFile(path.join(rootDir, 'templates', ...segments))
}

export const ensureImportLine = async (filePath: string, importLine: string): Promise<void> => {
  const content = await readFile(filePath)
  if (content.includes(importLine)) return
  await writeFile(filePath, `${importLine}\n${content}`)
}

export const applyToastToApp = async (projectPath: string): Promise<void> => {
...
export const configureTailwind = async (projectPath: string): Promise<void> => {
...
export const configureWebcam = async (projectPath: string): Promise<void> => {
  await ensureDir(path.join(projectPath, 'src', 'pages'))
  await writeFile(path.join(projectPath, 'src', 'pages', 'Camera.jsx'), cameraContent)
}

export const configurePrinter = async (projectPath: string): Promise<void> => {
  await ensureDir(path.join(projectPath, 'src', 'pages'))
  await writeFile(path.join(projectPath, 'src', 'pages', 'Printer.jsx'), printerContent)
}

export const configureToast = async (projectPath: string): Promise<void> => {
...
export const ensureGitignoreWithEnv = async (projectPath: string): Promise<void> => {
...
export const configureEnv = async (projectPath: string): Promise<void> => {
  await Promise.all([
...
export const configureWebSocket = async (projectPath: string = process.cwd()): Promise<void> => {
  await ensureDir(path.join(projectPath, 'src', 'services'))
...
export const buttonContent = `import React from 'react'
...
const Button = ({
  ...
  return (
    <button
      ...
      className={\`inline-flex items-center justify-center gap-2 \${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} \${className}\`}
      {...props}
    >
...
export const wrapperContent = `import React from 'react'
...
const Wrapper = ({
  ...
  const isImage = Boolean(
    bg && (bg.startsWith('/') || bg.startsWith('http') || bg.startsWith('.') || bg.includes('.'))
  )
  ...
  return (
    <section
      className={\`relative h-screen w-screen bg-cover bg-center bg-no-repeat overflow-hidden \${!isImage && bg ? bg : ''} \${className}\`.trim()}
...
export const createPackageHandlers = ({ installPackages }: PackageHandlersOptions = {}) => ({
  tailwind: async (projectPath: string) => {
    if (installPackages) await installTailwind(projectPath)
    await configureTailwind(projectPath)
  },
  axios: async (projectPath: string) => {
    if (installPackages) {
...
export const readCurrentPackageJson = async (): Promise<any> => {
...
export const getDependencies = (packageJson: any): Record<string, string> => ({
  ...(packageJson.dependencies || {}),
  ...(packageJson.devDependencies || {}),
})

export const assertDevScript = async (packageJson: any): Promise<void> => {
