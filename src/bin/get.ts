...
const postInstallHandlers: Record<string, (projectPath: string) => Promise<void>> = createPackageHandlers({ installPackages: false })

export interface InstallPackagesOptions {
  dev?: boolean
}

const installPackages = async (packageNames: string[], options: InstallPackagesOptions): Promise<void> => {
  try {
